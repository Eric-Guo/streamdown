...
export const parseEChartsOption = (option: string): EChartsOption => {
  ...
  // Allow users to paste snippets ending with a trailing semicolon (e.g. `option = {...};`)
  ...
  try {
    return JSON.parse(normalized) as EChartsOption;
  } catch (jsonError) {
    try {
      ...
      const parsed = new Function(`return (${normalized});`)() as unknown;
